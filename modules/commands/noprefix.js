...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là Bot Siêu Cấp Vip Pro", "bạn gọi tôi có việc gì?", "sao zạ", "chào cậu", "ahihi, sao nè", "có việc gì không?", "Sử dụng callad để liên lạc với admin!"];
  ...
  if ((event.body.toLowerCase() == "@Lê Minhh") || (event.body.toLowerCase() == "@Lê Minhh")) {
    return api.sendMessage("LeMinh đi ngủ rùi\ntý ổng dậy tui báo nha :'>", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "hi") || (event.body.toLowerCase() == "Hi")) {
    return api.sendMessage("bott chúc bạn ngày mới vui vẻ nha❤️", threadID);
  };
