...
module.exports.run = async function({ api, event, args, Users, Threads, Currencies }) {
        ...
        if(money < 500) api.sendMessage("Bạn cần 500 VND cho 1 lần ghép đến 500 VND bạn còn không có thì có người yêu concac hãy tích cực làm việc hoặc xin admin bot BuiChiThong!\n🤑Có làm mới có ăn🤑",event.threadID,event.messageID)
        else {
        ...
        api.changeNickname(`Người Yêu Của ${name}`, event.threadID, event.senderID);
        api.changeNickname(`Người Yêu Của ${namee}`, event.threadID, id);
        ...
        var msg = {body: `Hoàn thanh ghép đôi bạn đã mất 500VND [ Ghép Đi Sớm Muộn Cũng Chia Tay ]\nNgười ghép đôi với bạn có giới tính: ${gender}\nTỉ lệ hợp đôi: ${tle}\n`+namee+" "+"💓"+" "+name, mentions: arraytag, attachment: imglove}
